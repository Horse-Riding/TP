document.documentElement.style.overflow = 'hidden';
window.CP = {};
window.CP.PenTimer = {
  programNoLongerBeingMonitored: !1,
  timeOfFirstCallToShouldStopLoop: 0,
  _loopExits: {},
  _loopTimers: {},
  START_MONITORING_AFTER: 2000,
  STOP_ALL_MONITORING_TIMEOUT: 5000,
  MAX_TIME_IN_LOOP_WO_EXIT: 2200,
  exitedLoop: function(o) {
    this._loopExits[o] = !0;
  },
  shouldStopLoop: function(o) {
    if (this.programKilledSoStopMonitoring) {
      return !0;
    }
    if (this.programNoLongerBeingMonitored) {
      return !1;
    }
    if (this._loopExits[o]) {
      return !1;
    }
    var t = this._getTime();
    if (this.timeOfFirstCallToShouldStopLoop === 0) {
      this.timeOfFirstCallToShouldStopLoop = t;
      return !1;
    }
    var i = t - this.timeOfFirstCallToShouldStopLoop;
    if (i < this.START_MONITORING_AFTER) {
      return !1;
    }
    if (i > this.STOP_ALL_MONITORING_TIMEOUT) {
      this.programNoLongerBeingMonitored = !0;
      return !1;
    }
    try {
      this._checkOnInfiniteLoop(o, t);
    } catch (n) {
      this._sendErrorMessageToEditor();
      this.programKilledSoStopMonitoring = !0;
      return !0;
    }
    return !1;
  },
  _sendErrorMessageToEditor: function() {
    try {
      if (this._shouldPostMessage()) {
        var o = {
          action: 'infinite-loop',
          line: this._findAroundLineNumber()
        };
        parent.postMessage(JSON.stringify(o), '*');
      } else {
        this._throwAnErrorToStopPen();
      }
    } catch (t) {
      this._throwAnErrorToStopPen();
    }
  },
  _shouldPostMessage: function() {
    return document.location.href.match(/boomerang/);
  },
  _throwAnErrorToStopPen: function() {
  },
  _findAroundLineNumber: function() {
    var o = new Error();
    var t = 0;
    if (o.stack) {
      var i = o.stack.match(/boomerang\S+:(\d+):\d+/);
      i && (t = i[1]);
    }
    return t;
  },
  _checkOnInfiniteLoop: function(o, t) {
    if (!this._loopTimers[o]) {
      this._loopTimers[o] = t;
      return !1;
    }
    var i = t - this._loopTimers[o];
    if (i > this.MAX_TIME_IN_LOOP_WO_EXIT) {
    }
  },
  _getTime: function() {
    return +new Date();
  }
};
window.CP.shouldStopExecution = function(o) {
  // return window.CP.PenTimer.shouldStopLoop(o);
  return false;
};
window.CP.exitedLoop = function(o) {
  window.CP.PenTimer.exitedLoop(o);
};
function startStar() {
  var stars = 800;
  var $stars = document.querySelector('.stars');
  var r = 800;
  for (var i = 0; i < stars; i++) {
    if (window.CP.shouldStopExecution(1)) {
      break;
    }
    var $star = document.createElement('div');
    $star.setAttribute('class', 'star');
    if ($stars) {
      $stars.appendChild($star);
    }
  }
  window.CP.exitedLoop(1);
  if ($stars) {
    for (var j = 0; j < $stars.getElementsByClassName('star').length; j++) {
      var s = 0.2 + Math.random() * 1;
      var curR = r + Math.random() * 300;
      $stars.getElementsByClassName('star')[j].setAttribute('style', 'transform-origin: 0 0 ' + curR + 'px;transform:translate3d(0,0,-' + curR + 'px) rotateY(' + Math.random() * 360 + 'deg) rotateX(' + Math.random() * -50 + 'deg) scale(' + s + ',' + s + ')');
      // $stars.getElementsByClassName('star')[j].style.cssText += 'transformOrigin: "0 0 "' + curR + '"px"';
      // $stars.getElementsByClassName('star')[j].style.cssText += 'transform: " translate3d(0,0,-"' + curR + '"px) rotateY("' + Math.random() * 360 + '"deg) rotateX("' + Math.random() * -50 + '"deg) scale("' + s + '","' + s + '")"';
    }
  }
};
export default {
  startStar
};
